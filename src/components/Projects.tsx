...
function ProjectCard({ project }: { project: Project }) {
  return (
    <TiltCard className="rounded-2xl">
      <article className="overflow-hidden rounded-2xl border border-line bg-canvas transition-[border-color,box-shadow] duration-300 hover:border-accent-line hover:shadow-[0_24px_60px_-32px_rgba(11,18,32,0.45)]">
        {project.image ? (
          ...
          <ul className="mt-6 flex flex-wrap gap-2">
            {project.tags.map((tag, i) => (
              <motion.li
                ...
                whileHover={{ y: -2 }}
                className="cursor-default rounded-full border border-line bg-surface px-2.5 py-1 text-xs text-muted transition-colors hover:border-accent-line hover:text-accent"
              >
                {tag}
              </motion.li>
            ))}
            ...
            <div className="mt-6 flex items-center gap-5 border-t border-line pt-5">
              {project.github ? (
                <a
                  ...
                  className="group/link inline-flex items-center gap-1.5 text-sm font-medium text-body transition-colors hover:text-accent"
                >
...
export default function Projects() {
  return (
    <Section
      ...
      intro="Systems I designed and shipped. Each diagram animates how the system actually behaves, rather than showing a screenshot of client data."
    >
